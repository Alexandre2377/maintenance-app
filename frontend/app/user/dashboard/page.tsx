...
export default function UserDashboard() {
  ...
  const fetchMyRequests = async () => {
    ...
    try {
      const token = localStorage.getItem('access_token')
      const response = await axios.get(`${API_URL}/api/my-requests`, {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-gray-900">내 유지보수 요청</h1>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-8 text-center">
                    <div className="text-gray-500">
                      <p className="mb-4">아직 등록된 요청이 없습니다.</p>
                      <button
                        onClick={() => router.push('/request')}
        ...
        <div className="mt-6 text-center text-gray-600">
          총 <span className="font-bold text-primary-600">{requests.length}</span>개의 요청
        </div>
      </div>
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <span className={`mt-1 inline-block px-3 py-1 rounded-full text-xs font-semibold ${getCategoryBadgeColor(selectedRequest.category)}`}>
                    {getCategoryText(selectedRequest.category)}
                  </span>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-gray-500">우선순위</h3>
                  <span className={`mt-1 inline-block px-3 py-1 rounded-full text-xs font-semibold ${getPriorityBadgeColor(selectedRequest.priority)}`}>
              ...
              <div>
                ...
                <span className={`mt-1 inline-block px-3 py-1 rounded-full text-xs font-semibold ${getStatusBadgeColor(selectedRequest.status)}`}>
